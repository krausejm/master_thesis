...
\begin{figure}[htbp]
	...
	\label{fig:acc}
\end{figure}
In total, $9\cdot10^3$ $\eta'$ events were extracted which nonetheless still contain background contaminations. In order to take this into account in the later analysis the fraction of background is determined for each bin in beam energy and meson polar angle as the fraction of integrated background (all but $\eta'$) MC to total MC within the cut ranges in the invariant mass spectra.
...
Since the total number of events is proportional to the cross section $\sigma$, branching ratio BR and acceptance $\tilde{\epsilon}$, the ratio of reconstructed background to signal events is then given by $$R=\frac{\sigma\cdot\text{BR}\cdot\tilde{\epsilon}}{\sigma_{\eta'}\cdot\text{BR}_{\eta'\to\gamma\gamma}\cdot\tilde{\epsilon}_{\eta'}}.$$
