...
To evaluate the likelihood that the background contributions are made up of $2\pi^0$ or $\pi^0\eta$ production events the respective production cross sections and branching ratios (BR) to purely photonic final states are examined, see table \ref{tab:plausmc}. Both possible background reactions exceed $\eta'$ photoproduction in cross section and also in relative frequency of purely photonic final states. 

\begin{table}[htbp]
	\centering
	\begin{tabular}{ccc}
		\toprule
		reaction & $\sigma$ / \si{\micro\barn} & BR to $n\gamma$\\
		\hline
		$\gamma p\to p\eta'$ &1 & $0.02$\\
		$\gamma p\to p2\pi^0$ &5 &$0.98^2=0.96$\\
		$\gamma p\to p\pi^0\eta$ &3 &$0.98\cdot0.38=0.37$\\
		\bottomrule 		
	\end{tabular}
\caption{Total cross sections $\sigma$ and branching ratios (BR) to $n\gamma$ final states for signal and possible background contributions}
\label{tab:plausmc}
\end{table}
